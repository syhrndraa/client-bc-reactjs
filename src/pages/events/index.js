...
function EventPage() {
  ...
  }, [dispatch, events.keyword, events.category, events.talent]); //event akan berukan jika keyword, categor, atau talent berubah
  ...
  const handleChangeStatus = (id, status) => {
    const changedS = {
      statusCh: status === 'Published' ? 'Draft' : 'Published',
    };
    Swal.fire({
      title: `Apa kamu yakin ingin mengubah status ${status} menjadi ${changedS.statusCh}?`,
      text: '',
      icon: 'warning',
      showCancelButton: true,
      confirmButtonColor: '#3085d6',
